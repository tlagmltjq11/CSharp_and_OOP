...
하위 클래스에서 이용할 공통된 메소드를 상위 클래스가 제공해 주고, 하위 클래스는 상위 클래스에서 없는 기능을 추가하거나 수정하는 것이다.<br>
...
```
영희도 철수와 마찬가지로 이미 있던 기능을 재정의할 수도 있고, 새롭게 추가할 수도 있을 것입니다.
...
```
상속을 통해 다형성을 구현하면, 반복문을 통해 공통된 메소드들을 딱 한 번씩만 코드로 작성하면 됩니다.
